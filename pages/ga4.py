...
try:
    ...
    tmp_ga4 = get_data_from_bucket(bucket_name='dashboard_marketing_processed', file_name='Processed_concat.parquet', file_type='.parquet')
    raw_ga4 = pd.read_parquet(BytesIO(tmp_ga4), engine='pyarrow')
    st.session_state['ga4'] = raw_ga4
    ga4 = st.session_state['ga4']

ga4 = ga4.loc[~((ga4['event_page_location'].str.contains('hub')) & (ga4['event_name'] != 'purchase'))] #Removendo eventos do hub com excessão de vendas que possam ter saído
ga4 = ga4.loc[~ga4['Path'].str.contains(r'\bcurso\b', case=False)] #Removendo algum curso que disparou na propriedade
ga4['count'] = 1
########################## FILTERS ###############################################
...
with path_details:
    ...
    with inner_col1:
        tmp = details_path_data[['utm_source_std', 'default_channel','utm_content','count']].groupby(by=['utm_source_std', 'default_channel','utm_content']).sum().reset_index()
    ...
    with inner_col2:
        ...
        current_channels_data = limited_ga4.loc[limited_ga4['default_channel'] == selected_channel, ['default_channel', 'event_date', 'count']].groupby(by=['event_date', 'default_channel']).sum()
        current_channels_data['Periodo'] = 'Atual'
        comparison_channels_data = limited_benchmark.loc[limited_benchmark['default_channel'] == selected_channel, ['default_channel', 'event_date', 'count']].groupby(by=['event_date', 'default_channel']).sum()
...
with trails_expander:
    ...
    with tcol_1:
        tmp = dip[['utm_source_std', 'default_channel','utm_content','count']].groupby(by=['utm_source_std', 'default_channel','utm_content']).sum().reset_index()
    ...
    with tcol_2:
        tmp = pyof[['utm_source_std', 'default_channel','utm_content','count']].groupby(by=['utm_source_std', 'default_channel','utm_content']).sum().reset_index()
    ...
    with tcol_3:
        tmp = dsml[['utm_source_std', 'default_channel','utm_content','count']].groupby(by=['utm_source_std', 'default_channel','utm_content']).sum().reset_index()
    ...
    with tcol_4:
        tmp = quant[['utm_source_std', 'default_channel','utm_content','count']].groupby(by=['utm_source_std', 'default_channel','utm_content']).sum().reset_index()
